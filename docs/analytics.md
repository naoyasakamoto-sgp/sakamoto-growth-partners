...
`assets/site-analytics.js` uses the existing `window.gtag` initialized in each page head and falls back to `dataLayer` when Google Tag is unavailable. The measurement ID remains in the page-level Google Tag only and is not duplicated in event scripts.
...
- `insight_view`: `article_slug`, `article_title`, `category`
- `insight_50_percent`: `article_slug`, `article_title`, `category`
- `insight_90_percent`: `article_slug`, `article_title`, `category`
- `insight_cta_click`: `article_slug`, `article_title`, `category`, `cta_type`, `destination`
- `insight_related_article_click`: `article_slug`, `article_title`, `category`, `destination`
- `insight_service_click`: `article_slug`, `article_title`, `category`, `destination`
- `diagnosis_click`: `article_slug`, `article_title`, `category`, `destination`
